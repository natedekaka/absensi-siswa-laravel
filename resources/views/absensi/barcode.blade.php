...
<head>
    ...
    <title>Absensi Barcode - Absensi Siswa</title>
    ...
    <style>
        ...
        .header-card h2 { color: #1f2937; font-weight: 700; font-size: 1.5rem; margin-bottom: 5px; }
        .header-card p { color: #6b7280; font-size: 0.95rem; }
        ...
        .reader-placeholder i { font-size: 60px; color: var(--primary); margin-bottom: 15px; opacity: 0.5; }
        .status-buttons {
            display: grid;
        ...
        .status-btn:hover { border-color: var(--primary); transform: translateY(-3px); }
        .status-btn.selected { border-color: var(--primary); background: rgba(79, 70, 229, 0.1); }
        .status-btn i { font-size: 24px; }
        .status-btn.hadir { color: #10b981; }
        .status-btn.sakit { color: #f59e0b; }
        .status-btn.izin { color: #3b82f6; }
        .status-btn.alfa { color: #ef4444; }
        ...
        .btn-scan:hover { transform: translateY(-2px); box-shadow: 0 10px 20px rgba(0, 0, 0, 0.15); color: white; }
        .btn-absen {
            width: 100%;
            padding: 16px;
            border-radius: 14px;
        ...
        .result-card .siswa-info { display: flex; align-items: center; gap: 20px; margin-bottom: 20px; }
        .result-card .avatar {
            width: 70px;
            height: 70px;
            border-radius: 20px;
        ...
        .result-card .details h4 { color: #1f2937; font-weight: 700; margin-bottom: 5px; }
        .result-card .details p { color: #6b7280; margin: 0; font-size: 0.9rem; }
        .status-badge { display: inline-flex; align-items: center; gap: 8px; padding: 12px 20px; border-radius: 14px; font-weight: 600; font-size: 1rem; }
        .status-badge.sudah { background: #fef3c7; color: #92400e; }
        ...
        .footer-link a { color: white; text-decoration: none; font-weight: 500; }
        .footer-link a:hover { text-decoration: underline; }
        @media (max-width: 480px) {
            body { padding: 10px; }
            .header-card, .scanner-card, .result-card { padding: 20px; border-radius: 16px; }
            .header-card h2 { font-size: 1.25rem; }
...
<body>
    ...
    <div class="container">
        <div class="header-card">
            ...
            <div class="mt-3 p-2 bg-white bg-opacity-50 rounded">
                <form method="GET" class="d-flex gap-2 flex-wrap justify-content-center align-items-center">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <input type="date" name="tgl" id="tgl" class="form-control form-control-sm" value="{{ $today }}" style="width: 140px;">
                    </div>
                    <div class="d-flex align-items-center gap-2">
                        <i class="fas fa-school"></i>
                        <select name="semester_id" id="semesterSelect" class="form-select form-select-sm" style="width: 180px;">
                            <option value="">Semua Semester</option>
                            @foreach($semesters as $s)
                            <option value="{{ $s->id }}" {{ $semesterAktif && $s->id == $semesterAktif->id ? 'selected' : '' }}>
                                {{ $s->nama }}
                            </option>
                            @endforeach
                        </select>
                    </div>
                    <button type="button" class="btn btn-sm btn-primary" onclick="filterData()">
            ...
            <div class="mt-3 d-flex justify-content-center gap-3 flex-wrap">
                ...
                <span class="badge bg-info text-white">Izin: {{ $stats['Izin'] }}</span>
                <span class="badge bg-danger">Alfa: {{ $stats['Alfa'] }}</span>
                <span class="badge bg-secondary">Terlambat: {{ $stats['Terlambat'] }}</span>
                <span class="badge bg-dark">Total: {{ array_sum($stats) }}</span>
            </div>
        </div>
        ...
        <div class="result-card mt-3">
            <h5 class="mb-3"><i class="fas fa-history me-2"></i>Riwayat Scan ({{ $riwayat->count() }})</h5>
                ...
                <table class="table table-sm table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $r->nama }}</td>
                            <td>{{ $r->nama_kelas ?? '-' }}</td>
                            <td><span class="badge bg-{{ $r->status == 'Hadir' ? 'success' : ($r->status == 'Sakit' ? 'warning' : ($r->status == 'Izin' ? 'info' : 'danger')) }}">{{ $r->status }}</span></td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
    ...
    <div class="footer-link">
        <a href="{{ route('absensi.index') }}">← Kembali ke Absensi Manual</a>
    </div>
    ...
    <script>
        ...
        let currentTgl = '{{ $today }}';
        let currentSemesterId = {{ $semesterAktif ? $semesterAktif->id : 'null' }};
        ...
        function cariSiswa(barcode) {
            ...
            fetch('{{ route("absensi.cari-siswa") }}?barcode=' + encodeURIComponent(barcode) + '&tgl=' + currentTgl + '&semester_id=' + (currentSemesterId || ''))
        ...
        function showSiswaInfo(data) {
            ...
            if (data.sudah_absen) {
                statusInfo.innerHTML = '<div class="status-badge sudah"><i class="fas fa-check-circle"></i>Siswa sudah absen hari ini (' + data.status_display + ')</div>';
        ...
        function showError(message) {
            ...
            resultSection.innerHTML = `
                <div class="text-center">
                    ...
                    <button class="btn-absen" id="btnCobaLagi" style="background: #6b7280;">
        ...
        function simpanAbsensi() {
            ...
            formData.append('tgl', currentTgl);
            formData.append('semester_id', currentSemesterId || '');

            fetch('{{ route("absensi.simpan-barcode") }}', {
                method: 'POST',
                headers: {
        ...
        function filterData() {
            currentTgl = document.getElementById('tgl').value;
            currentSemesterId = document.getElementById('semesterSelect').value || null;
            location.href = '{{ route("absensi.barcode") }}?tgl=' + currentTgl + '&semester_id=' + (currentSemesterId || '');
        }
    </script>
</body>
</html>
